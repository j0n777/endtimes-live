...
const TacticalRadar: React.FC<TacticalRadarProps> = ({ events }) => {
  ...
  useEffect(() => {
    ...
    // Draw Crosshairs
    g.append("line").attr("x1", 0).attr("y1", -radius).attr("x2", 0).attr("y2", radius).attr("stroke", "#34d399").attr("opacity", 0.3);
    g.append("line").attr("x1", -radius).attr("y1", 0).attr("x2", radius).attr("y2", 0).attr("stroke", "#34d399").attr("opacity", 0.3);
    ...
    events.forEach(event => {
      ...
      const color = event.severity === 'HIGH' ? '#ef4444' : event.severity === 'ELEVATED' ? '#f59e0b' : '#34d399';
